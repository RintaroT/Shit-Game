...
var empty = [];
for (var i = 0; i < 50; i++) {
  var treex = Math.round(Math.random() * 50);
  var treey = Math.round(Math.random() * 50);
  tree[tree.length] = treex + "," + treey;
 treex++;
  tree[tree.length] = treex + "," + treey;
   treex--;
   treex--;
  ...
  for (var i = 0; i < tree.length; i++) {
    if (document.getElementsByClassName(tree[i])[0]) {
      if (document.getElementsByTagName("TD")[p].className !== document.getElementsByClassName(tree[i])[0].className) {
        empty[empty.length] = document.getElementsByTagName("TD")[p].className;
      }
    }
  }
}
...
sheet.addRule(".c0c0", "display: none;", 0);
